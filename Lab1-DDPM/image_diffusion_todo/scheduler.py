...
class BaseScheduler(nn.Module):
    def __init__(
        ...
        if mode == "linear":
            ...
            T = self.num_inference_timesteps
            s = 0.008
...
class DDPMScheduler(BaseScheduler):
    ...
    def step_predict_noise(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        x0_pred = (
            x_t - (1 - alpha_bar_t).sqrt() * eps_theta
        ) / alpha_bar_t.sqrt()
        ...
        sample_prev = mean + nonzero_mask * variance.sqrt() * noise
        #######################
        return sample_prev
    ...
    def step_predict_mean(self, x_t: torch.Tensor, t: int, mean_theta: torch.Tensor):
        ...
        sample_prev = mean_theta + nonzero_mask * variance.sqrt() * noise
        #######################
        return sample_prev
